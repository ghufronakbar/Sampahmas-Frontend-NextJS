...
const english = {
  ...
  GREEN_MARKET_DESCRIPTION:
    "The marketplace that we provide for users to enjoy the points that have been collected then users can also sell their own products which can be in the form of processed plastic waste goods or not processed plastic.",
  EXPLORE: "Explore",
  ABOUT_US: "About Us",
  ...
  FIND_VENDING: [
    "Find Vending",
    "Find a Vending Machine around you, don't miss it!",
  ],
  EXCHANGE: [
    "Exchange",
    "Redeem your plastic bottles into Sampay points and Enjoy the Rewards",
  ],
  ENJOY_SHOPPING: [
    "Enjoy Shopping",
...
const indonesian = {
  ...
  GREEN_MARKET_DESCRIPTION:
    "Marketplace yang kami sediakan untuk para pengguna agar dapat menikmati poin yang sudah dikumpulkan kemudian para pengguna juga dapat menjual produk mereka sendiri yang dapat berupa barang barang olahan limbah plastik maupun buka olahan plastik.",
  EXPLORE: "Jelajahi",
...
const useText = () => {
  ...
  useEffect(() => {
    if (router.isReady) {
      if (lang !== "en" && lang !== "id") {
        router.push(
          `/en/${router.asPath.split("/").slice(2).join("/")}`,
          undefined,
          { shallow: true }
        );
      }
  ...
  const result =
    language === "en" ? english : language === "id" ? indonesian : english;
  return result;
};
